/* 말씀읽기APP — Firebase 로그인/진도저장 + bible.json
   ...
   + 절 자동이동/장 자동이동(성공 처리)
   + "해당절읽음" 버튼 지원
   + 마이크 ON일 때 음성모드 변경 금지(라디오 없으면 자동 무시)
*/
(() => {
  // ---------- PWA ----------
  ...
  function initFirebase() {
    ...
    auth = app.auth();
    db = app.firestore();
    console.log("[Firebase] 초기화 OK (apps:", firebase.apps.length, ")");
  }
  initFirebase();
  ...
  function showScreen(name) {
    if (name === "login") {
      scrLogin?.classList.add("show");
      scrApp?.classList.remove("show");
    } else {
      scrApp?.classList.add("show");
      scrLogin?.classList.remove("show");
    }
  }
  ...
  const state = {
    ...
    myStats: {
      versesRead: 0,
      chaptersRead: 0,
      last: { bookKo: null, chapter: null, verse: 0 },
    },
    ignoreUntilTs: 0,
    paintedPrefix: 0,
    verseDoneMap: {}, // key: `${bookKo}#${chapter}` -> Set(verseIndex1Based)
  };
  ...
  async function loadBible() {
    try {
      ...
      els.verseText &&
        (els.verseText.textContent = "루트에 bible.json 파일이 필요합니다.");
    }
  }
  loadBible();
  ...
  function mapAuthError(e) {
    ...
    if (code.includes("operation-not-allowed"))
      return "이메일/비밀번호 로그인이 비활성화되어 있습니다. 콘솔에서 활성화해주세요.";
  ...
  function keyForChapter() {
    return `${state.currentBookKo}#${state.currentChapter}`;
  }
  ...
  async function primeMicrophone() {
    if (primeStream && primeStream.getTracks().some((t) => t.readyState === "live"))
      return primeStream;
    try {
  ...
  const NUM_KO = {
    영: 0, 공: 0, 하나: 1, 한: 1, 둘: 2, 두: 2, 셋: 3, 세: 3, 넷: 4, 네: 4,
  ...
  function normalizeKoreanNumbers(s) {
    return s
      ...
      .replace(/십\s*(한|일)/g, "11")
      .replace(/십\s*(둘|이)/g, "12")
      .replace(/십\s*(셋|삼)/g, "13")
      .replace(/십\s*(넷|사)/g, "14")
      .replace(/십\s*(다섯|오)/g, "15")
      .replace(/십\s*(여섯|육)/g, "16")
      .replace(/십\s*(일곱|칠)/g, "17")
      .replace(/십\s*(여덟|팔)/g, "18")
  ...
  function prefixCoverage(targetJ, spokenJ) {
    ...
    const thrShort = 0.80, thrMedium = 0.78, thrLong = 0.75;
    let bestI = 0;
    for (let i = 1; i <= n; i++) {
  ...
  function makeRecognizer() {
    ...
    const r = new SR();
    r.lang = "ko-KR";
    r.continuous = !IS_ANDROID; // 안드로이드 false 권장
    r.interimResults = !IS_ANDROID ? true : false; // 안드로이드 final 위주
  ...
  function runRecognizerLoop() {
    ...
    recog.onresult = (evt) => {
      ...
      // 화면 채움(앞서가지 않도록 점진)
      const paintLen = Math.min(
        targetJ.length,
        Math.max(state.paintedPrefix, Math.min(matched, state.paintedPrefix + 3))
      );
      paintRead(paintLen);
  ...
  function refreshRecogModeLock() {
    ...
    if (!radios?.length) return; // 없으면 아무것도 하지 않음
  ...
  async function loadLeaderboard() {
    ...
    list.forEach((u, idx) => {
      ...
      const v = Number(u.versesRead || 0),
        c = Number(u.chaptersRead || 0);
  ...
  window.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && els.matrixModal?.classList.contains("show")) closeMatrix();
  });
